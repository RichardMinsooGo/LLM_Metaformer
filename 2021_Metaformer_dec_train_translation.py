...
if os.path.isfile('./checkpoints/Metaformer.pt'):
    model.load_state_dict(torch.load('./checkpoints/Metaformer.pt'))
...
for epoch in range(N_EPOCHS):
    ...
    if train_loss < best_epoch_loss:
        ...
        torch.save(model.state_dict(), './checkpoints/Metaformer.pt')
...
trained_model.load_state_dict(torch.load('./checkpoints/Metaformer.pt'))
